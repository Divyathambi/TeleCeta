...
class _$ValueFailureTearOff {
  ...
  _ExceedingLength<T> exceedingLength<T>({T? failedValue, int? max}) {
  ...
  _InvalidPassword<T> invalidPassword<T>({T? failedValue}) {
...
class _$_ExceedingLength<T> implements _ExceedingLength<T> {
  const _$_ExceedingLength({this.failedValue, this.max});
...
abstract class _ExceedingLength<T> implements ValueFailure<T> {
  const factory _ExceedingLength({T? failedValue, int? max}) =
...
class _$_InvalidPassword<T> implements _InvalidPassword<T> {
  const _$_InvalidPassword({this.failedValue});
...
abstract class _InvalidPassword<T> implements ValueFailure<T> {
  const factory _InvalidPassword({T? failedValue}) = _$_InvalidPassword<T>;
